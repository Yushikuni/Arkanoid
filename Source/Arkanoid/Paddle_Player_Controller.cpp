...
void APaddle_Player_Controller::SetupInputComponent()
{
	...
	EnableInput(this); //detekovani inputu pri stisknuti

	

	InputComponent->BindAxis("MoveHorizontal", this, &APaddle_Player_Controller::MoveHorizontal); //spusobi to ze se hrac zacne pohybovat horizontalne pri stisknuti
...
{
	auto MyPawn = Cast<APaddle>(GetPawn()); //auto automacky detekuje typ promenne a sam si ji nastavi
...
void APaddle_Player_Controller::SpawnNewBall()
{
	...
	if (BallObj)
	{
		MyBall = GetWorld()->SpawnActor<ABall>(BallObj, SpawnLocation, SpawnRotation, SpawInfo);
	}
}
